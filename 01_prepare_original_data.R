...
# Read Data ---------------------------------------------------------------
flow <- read_csv('Data/Input_Data/Original/tile_flow.csv', guess_max = 80000)

weather <- read_csv('Data/Input_Data/Original/daily_tile_flow.csv')
...
data <-
  ...
  full_join(weather, by = c('plotid', 'date')) %>%
  select(id, siteid, plotid, year, date, tile_flow, tile_flow_filled, precip_on_site) 
...
data_to_analyze <-
  data %>%
  select(siteid, plotid, year, date, flow = tile_flow_filled, rain = precip_on_site) %>%
